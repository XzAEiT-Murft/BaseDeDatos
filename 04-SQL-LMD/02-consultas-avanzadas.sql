...
SELECT p.Num_Pedido AS [N�mero de Pedido], 
	p.Importe AS [Total a Pagar], 
	c.Empresa AS [Cliente], 
	c.Limite_Credito AS [L�mite de Cr�dito]
 FROM Pedidos AS p
 INNER JOIN 
 Clientes AS c
...
SELECT p.Num_Pedido AS [N�mero de Pedido], 
 ...
 SELECT ProductID AS [N�mero de Producto],
 ...
 SELECT pr.ProductID AS [N�mero del producto],
...
-- listar los pedidos, mostrando el numero de pedido, el importe y la descripci�n de los productos
 ...
 SELECT p.Num_Pedido AS [N�mero del pedido],
		p.Importe AS [Total],
 ...
 --�Cual es la cuota menor?
  ...
  --Seleccionar la fecha de pedido m�s antigua
  SELECT MIN(Fecha_Pedido) AS [Fecha de pedido m�s antigua]
  FROM Pedidos
 ...
 SELECT COUNT(Num_Pedido) AS [N�mero de pedidos]
 FROM Pedidos
 WHERE Importe > 25000
 ...
 INNER JOIN Representantes AS r
 ON p.Rep = r.Num_Empl
 WHERE r.Nombre in ('Tom�s Saz' , 'Mar�a Jim�nez', 'Pablo Cruz')
 GROUP BY Nombre
 ORDER BY Nombre
 GO
...
-- 1) Seleccionar el ingreo total por cliente en 1997 y 
...
--    enviados a alemania ordenados por categoria asendente y dentro de categoria
...
-- 3) seleccionar los empleados con mas pedidos realizados por anio
--    ordenados por anio y numero de pedidos
...
ORDER BY [Anio], [Número de Pedidos] DESC;
